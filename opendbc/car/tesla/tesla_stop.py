...
class TeslaStop:
  ...
  def update(self, CC, CS, accel):
    ...
    if not self.stop_line_distances:
      return accel
    ...
    time = np.interp(v_ego, [20 * CV.KPH_TO_MS, 80 * CV.KPH_TO_MS], [2.75, 4])
    if is_effective_red and ((avg_stop_line_distance / v_ego) <= time or self.phase != 0):
      ...
      self.required_decelerations = self.required_decelerations[-10:]  # Keep last 10 values

      if not self.required_decelerations:
        return accel

      avg_required_decelerations = sum(self.required_decelerations) / len(self.required_decelerations)

      output_accel = 0

      if self.phase == 0:
        output_accel = avg_required_decelerations
