...
class CarState(CarStateBase):
  ...
  def update_raven(self, can_parsers) -> structs.CarState:
    ...
    # Vehicle speed
    ret.vEgoRaw = cp_chassis.vl["DI_torque2"]["DI_vehicleSpeed"] * CV.KPH_TO_MS
    ret.vEgo, ret.aEgo = self.update_speed_kf(ret.vEgoRaw)
    ...
    autopark_state = self.can_defines["DI_state"]["DI_autoparkState"].get(int(cp_pt.vl["DI_state"]["DI_autoparkState"]), None)
    cruise_enabled = cruise_state in ("ENABLED", "STANDSTILL", "OVERRIDE", "PRE_FAULT", "PRE_CANCEL")
    self.update_autopark_state(autopark_state, cruise_enabled)
