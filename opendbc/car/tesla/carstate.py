...
class CarState(CarStateBase):
  ...
  def update(self, can_parsers) -> structs.CarState:
    ...
    eac_error_code = self.can_defines[f"{epas_name}_sysStatus"][f"{epas_name}_eacErrorCode"].get(int(epas_status[f"{epas_name}_eacErrorCode"]), None)
    ret.steeringDisengage = self.hands_on_level >= 3 or (eac_status == "EAC_INHIBITED" and
                                                         eac_error_code == "EAC_ERROR_HIGH_ANGLE_RATE_SAFETY")

    # Cruise state
    di_state = cp_party.vl["DI_state"] if is_3Y else cp_chassis.vl["DI_state"]
    ...
    #autopark_state = self.can_defines["DI_state"]["DI_autoparkState"].get(int(cp_party.vl["DI_state"]["DI_autoparkState"]), None)
