...
class TeslaStop:
    ...
    def reset(self):
      self.phase = 0
      self.required_decelerations = [0] * 25
    ...
    def update(self, CC, CS, accel):
        ...
        if is_effective_red and ((stop_line_distance / v_ego) <= time or self.phase != 0):
            ...
            self.required_decelerations = self.required_decelerations[-max(int(stop_line_distance), 35):]
            ...
            if stop_line_distance < 12 and self.phase == 0:
                self.phase = 1
